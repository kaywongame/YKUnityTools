...
[ExecuteInEditMode]
public class UTVertexVisualizer_2 : MonoBehaviour
{
   ...
   void Start()
   {
      ...
      {
         m_EndVertIdx = Mathf.Min(m_Mesh.vertices.Length, m_StartVertIdx + m_VertRange);
         m_VertRange = m_EndVertIdx - m_StartVertIdx;

         m_EndTriIdx = Mathf.Min(m_Mesh.triangles.Length, m_StartTriIdx + m_TriRange);
         m_TriRange = m_EndTriIdx - m_StartTriIdx;
      }
   ...
   void OnGUI()
   {
      ...
      if (m_ShowTriangleNo)
      {
         ...
         int maxTri = Mathf.Min(triNum, m_EndTriIdx );
         int minTri = 0 > m_StartTriIdx ? 0 : m_StartTriIdx;
         minTri = maxTri < minTri ? maxTri : minTri;
         
         for (int j = minTri; j < maxTri; j++)
         {
            /*
            print("tri " + j + " V idx : [ " + 
            ...
            */
            ...
            GUI.Label(new Rect(screenPos2.x, screenPos2.y, triNumStr.Length * 10, 20),
               triNumStr,m_guiStyleForTriText );
         }
      }
   }
