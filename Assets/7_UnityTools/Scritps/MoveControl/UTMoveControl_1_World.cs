...
/// </summary>
public class UTMoveControl_1_World : MonoBehaviour
{
   void Update ()
   {
      ...
      transform.position += transform.TransformDirection (moveVec * Time.deltaTime) ;
      ...
      Debug.DrawLine(offsetPos2, offsetPos2 +  transform.TransformDirection (xVec), Color.red);

      // 파란색으로 정규화 되지 않은 Z축 이동 방향 벡터를 그려준다.
      Debug.DrawLine(offsetPos, offsetPos + transform.TransformDirection (zVec), Color.blue);

      // 자홍색으로 정규화 되지 않은 이동 방향 벡터를 그려준다.
      Debug.DrawLine(offsetPos, offsetPos + transform.TransformDirection (dirVec), Color.magenta);

      // 흰색으로 정규화 된 이동 벡터를 그려준다.
      Debug.DrawLine(transform.position, transform.position + transform.TransformDirection (moveVec),
         Color.white);
